...
/*   Updated: 2019/04/05 13:19:10 by tmatthew         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		find_subst(char *input, int x, t_lctx *ctx)
{
	...
	while ((next = next_missing_symbol(internal_missing)))
	{
		if (input[x + i] && input[x + i] == '\\')
			i += 2;
		else if (input[x + i] && input[x + i] == '$' && input[x + i + 1] && (
		...
		else if (input[x + i] && next == BRACE_SUB && input[x + i] == '}')
		{
			pop_missing_symbol(&internal_missing);
			i += 1;
		}
		else if (input[x + i] && (next == CMD_SUB || next == MATH_SUB) && input[x + i] == ')')
		{
			pop_missing_symbol(&internal_missing);
			i += 1;
		}
		else if (next == VAR_SUB && !IS_VAR_CHAR(input[x + i]))
			pop_missing_symbol(&internal_missing);
